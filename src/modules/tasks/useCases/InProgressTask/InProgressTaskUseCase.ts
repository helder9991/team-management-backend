...
class InProgressTaskUseCase {
  ...
  async execute({ id, userTeamId }: ICreateTaskParams): Promise<Task> {
    const taskStatusExists =
      await this.taskStatusRepository.findByName(inProgressTaskStatus)
    ...
    if (taskExists.taskStatusId === taskStatusExists.id)
      throw new AppError('This tasks was already in progress.')
...
export default InProgressTaskUseCase
