...
describe('Ready Task E2E', () => {
  ...
  beforeEach(async () => {
    try {
      ...
      const taskBody: ICreateTaskControllerResponse =
        response.body as ICreateTaskControllerResponse

      await request(app)
        .post(`/task/${taskBody.id}/complete`)
        .set('Authorization', `Bearer ${teamMemberToken}`)
        .send(task)
  ...
  it('Should be able to ready a task', async () => {
    const response = await request(app)
      .post(`/task/${createdTask.id}/ready`)
      .set('Authorization', `Bearer ${teamMemberToken}`)

    expect(response.status).toBe(204)
  })

  it('Shouldn`t be able to ready a new task with a non-team-member account', async () => {
  ...
  it('Should`t be able to ready a task if you pass a wrong parameters', async () => {
    const response = await request(app)
      .post(`/task/${1}/ready`)
      .set('Authorization', `Bearer ${teamMemberToken}`)
