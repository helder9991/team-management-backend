...
describe('In Progress Task E2E', () => {
  ...
  it('Should be able to mark a task to in progress', async () => {
    const response = await request(app)
      .post(`/task/${createdTask.id}/in-progress`)
      .set('Authorization', `Bearer ${teamMemberToken}`)

    expect(response.status).toBe(204)
  })

  it('Shouldn`t be able to mark a task to in progress with a non-team-member account', async () => {
  ...
  it('Should`t be able to mark a task to in progress if you pass a wrong parameters', async () => {
    const response = await request(app)
      .post(`/task/${1}/in-progress`)
      .set('Authorization', `Bearer ${teamMemberToken}`)
